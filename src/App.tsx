...
function App() {
  ...
  const calculate = () => {
    ...
    const newEspression = newParts.join(" ");
    if (isOperator(newEspression.charAt(0))) {
      setAnswer(eval(answer + newEspression) as string);
    } else {
      setAnswer(eval(newEspression) as string)
    }
    setExpression("");
  };
